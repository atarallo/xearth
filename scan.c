...
void
orth_find_edge_xing (prev, curr, rslt)
    double *prev;
    double *curr;
    double *rslt;
{
...
orth_handle_xings ()
{
    ...
    if (xings[0].type == XingTypeExit) {
        for (i = 0; i < nxings; i += 2) {
            ...
            if ((from->type != XingTypeExit) || (to->type != XingTypeEntry))
        ...
        if ((from->type != XingTypeExit) ||
            (to->type != XingTypeEntry) || (from->angle < to->angle))
...
void
orth_scan_arc (x_0, y_0, a_0, x_1, y_1, a_1)
    double x_0, y_0, a_0;
    double x_1, y_1, a_1;
{
    int i;
...
void
xing_error (file, line, idx, nxings, xings)
    const char *file;
    int line;
    int idx;
    int nxings;
