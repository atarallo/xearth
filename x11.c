...
init_x_colors ()
{
    ...
    if (mono) {
        ...
        for (i = 0; i < dither_ncolors; i++) {
            xc.red =
                ((1 << 16) - 1) * pow (((double) tmp[0] / 255), inv_xgamma);
            xc.green =
                ((1 << 16) - 1) * pow (((double) tmp[1] / 255), inv_xgamma);
            xc.blue =
                ((1 << 16) - 1) * pow (((double) tmp[2] / 255), inv_xgamma);
...
init_x_separate_window ()
{
    ...
    xearth_window = XCreateSimpleWindow (dsply,
                                         ...
                                         wdth,
                                         hght,
                                         DefaultBorderWidth, white, black);
    ...
    MarkerInfo *info;
{
    ...
    if (text != NULL) {
        len = strlen (text);
        XTextExtents (font, text, len, &direction, &ascent, &descent,
                      &extents);
...
static void
draw_outlined_string (dpy, pix, fg, bg, x, y, text, len)
    Display *dpy;
    Pixmap pix;
    Pixel fg;
    Pixel bg;
