...
compute_positions ()
{
    ...
    if (fixed_time == 0) {
        ...
        if (start_time == 0)
            ...
            current_time =
                start_time + (current_time - start_time) * time_warp;
    } else {
...
void
sun_relative_position (lat_ret, lon_ret)
 ...
 */
void
pick_random_position (lat_ret, lon_ret)
    double *lat_ret;    /* (return) latitude  */
    double *lon_ret;    /* (return) longitude */
{
    int i;
    ...
    char *argv[];
{
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp (argv[i], "-proj") == 0) {
            ...
            i += 1;
            if (i >= argc)
                usage ("missing arg to -size");
            decode_size (argv[i]);
        } else if (strcmp (argv[i], "-shift") == 0) {
            i += 1;
            if (i >= argc)
                usage ("missing arg to -shift");
            decode_shift (argv[i]);
            ...
            i += 1;
            if (i >= argc)
                usage ("missing arg to -markerfile");
            ...
            i += 1;
            if (i >= argc)
                usage ("missing arg to -overlayfile");
            overlayfile = argv[i];
        } else if (strcmp (argv[i], "-cloudfile") == 0) {
            i += 1;
            if (i >= argc)
                usage ("missing arg to -cloudfile");
            cloudfile = argv[i];
        } else if (strcmp (argv[i], "-gamma") == 0) {
            i += 1;
            if (i >= argc)
                usage ("missing arg to -gamma");
            ...
            i += 1;
            if (i >= argc)
                usage ("missing arg to -ncolors");
            decode_colors (argv[i]);
        } else if (strcmp (argv[i], "-font") == 0) {
            i += 1;
            if (i >= argc)
                usage ("missing arg to -font");
    ...
    const char *msg;
{
    ...
    fprintf (stderr, " [-proj proj_type] [-pos pos_spec] [-rot angle]\n"
             ...
             " [-grid2 grid2]\n"
             " [-day pct] [-night pct] [-term pct] [-gamma gamma_value]\n"
             " [-wait secs] [-timewarp factor] [-time fixed_time]\n"
             " [-onepix|-twopix] [-mono|-nomono] [-ncolors num_colors]\n"
             " [-font font_name] [-root|-noroot] [-geometry geom]"
             " [-title title]\n"
             " [-iconname iconname] [-name name] [-fork|-nofork]"
             " [-once|-noonce]\n"
             " [-nice priority] [-gif] [-png] [-jpeg] [-bmp] [-ppm]\n"
