...
void
writeinc (int height, int maxwidth)
{
    int maxbytes;
    int i, j, k;
...
void
dofnt (unsigned char *fnt, int fntsize)
{
    int version;
    int ftype;
    int off_facename;
...
void
nefon (unsigned char *fon, int fonsize, int neoff)
{
    int rtable;
    int shift;
    int p;
...
void
dofon (unsigned char *fon, int fonsize)
{
    int neoff;
    ...
    } else if (fon[neoff] == 'P' && fon[neoff + 1] == 'E'
               && fon[neoff + 2] == 0 && fon[neoff + 3] == 0) {
