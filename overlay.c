...
overlay_init ()
{
    ...
    if (overlayfile != NULL) {
        ...
        if (f != NULL) {
            ...
            default:
                fprintf (stderr,
                         "xearth: warning: unknown image file format: %s\n",
                         overlayfile);
                overlay = NULL;
                break;
            }
    ...
    if (cloudfile != NULL) {
        ...
        if (f != NULL) {
            ...
            for (x = 0; x < gdImageSX (clouds); x++) {
                if (gdImageAlpha
                    (clouds,
                     gdImageGetPixel (clouds, x,
                                      gdImageSY (clouds) / 2)) != 0) {
                    clouds_alpha = 1;
                    break;
                }
            }
...
int
overlay_pixel (double lat, double lon)
{
    int x, y;
    double r;
    int c;
    ...
    y = (int) (-lat * gdImageSY (overlay) / M_PI * r +
               gdImageSY (overlay) / 2);
    /* handle minor rounding errors */
    if (x == -1)
        x++;
    if (x == gdImageSX (overlay))
        x--;
    if (y == -1)
        y++;
    if (y == gdImageSY (overlay))
        y--;
    if (x < 0 || x >= gdImageSX (overlay) || y < 0 || y >= gdImageSY (overlay)) {
        return -1;
    }
    c = gdImageGetPixel (overlay, x, y);
    return PixRGB (gdImageRed (overlay, c), gdImageGreen (overlay, c),
                   gdImageBlue (overlay, c));
}

int
cloud_pixel (double lat, double lon, int p)
{
    int x, y;
    ...
    if (x == -1)
        x++;
    if (x == gdImageSX (clouds))
        x--;
    if (y == -1)
        y++;
    if (y == gdImageSY (clouds))
        y--;
        ...
        if (clouds_alpha) {
            ...
            return PixRGB (a * r / 127 +
                           ...
                           a * g / 127 + (127 - a) * gdImageGreen (clouds,
                                                                   c) / 127,
                           a * b / 127 + (127 - a) * gdImageBlue (clouds,
                                                                  c) / 127);
        } else {
            return PixRGB (r + gdImageRed (clouds, c) * (255 - r) / 255,
